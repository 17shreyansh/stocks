    ...
    const Downloads = () => {
      ...
      // Fetch categories after page data is loaded
      useEffect(() => {
        fetchCategories();
      }, [pageData]);
      ...
      const fetchDocuments = async () => {
        try {
          setLoading(true);
          // Only fetch from API if no admin-managed documents exist
      ...
      const fetchCategories = async () => {
        try {
          // First try to get categories from page data (like Policies page)
          if (pageData.categories && pageData.categories.length > 1) {
            setCategories(pageData.categories);
            return;
          }
          
          // Fallback to API categories if page data doesn't have them
      ...
      const filteredDocuments = useMemo(() => {
        // Use admin-managed documents if available, otherwise use API documents
        ...
        return allDocuments.filter(doc => {
          ...
          switch (sortBy) {
            ...
            case 'size': 
              const aSize = typeof a.fileSize === 'string' ? 0 : (a.fileSize || 0);
              const bSize = typeof b.fileSize === 'string' ? 0 : (b.fileSize || 0);
              return bSize - aSize;
            default: 
              return 0;
          }
      ...
      const handleDownload = (document, event) => {
        ...
        if (document.downloadUrl) {
          try {
            const httpsUrl = document.downloadUrl.replace('http://', 'https://');
